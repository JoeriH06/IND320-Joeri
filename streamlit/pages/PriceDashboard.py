...
# ---------------------- Header ---------------------------
st.title("📊 Price Dashboard — Elhub Production")
st.caption("Hourly production per group and price area (MongoDB-backed)")
...
# ---------------------- LEFT COLUMN -----------------------
with left:
    st.markdown("### Area & Composition")
    st.markdown("<hr>", unsafe_allow_html=True)

    price_areas = sorted(df["pricearea"].dropna().unique())

    # Horizontal radio = better UX
    area = st.radio(
        "Select price area",
        price_areas,
        index=0,
...
# ---------------------- RIGHT COLUMN ----------------------
with right:
    st.markdown("### Groups & Monthly Trend")
    st.markdown("<hr>", unsafe_allow_html=True)
...
with st.expander("ℹ️ About the data"):
    st.markdown(
        ...
        **Storage:** MongoDB (database: `elhub`, collection: `df_clean`)  

        **Variables:**
        - `pricearea` — price zone (NO1–NO5)  
        - `productiongroup` — HYDRO, WIND, SOLAR, THERMAL, OTHER  
        - `starttime` — hourly timestamp (UTC)  
        - `quantitykwh` — production (kWh)

        Charts show hourly production per area/group, aggregated as needed.
        """
    )
